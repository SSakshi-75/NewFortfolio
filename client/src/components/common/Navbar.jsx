...
const Navbar = () => {
  ...
  return (
    <nav className="w-full flex items-center justify-between py-4 px-6 md:px-10 fixed top-0 left-0 z-50 bg-gradient-to-b from-[#070b14]/90 to-transparent">
      ...
      {mobileOpen && (
        <div className="md:hidden absolute top-16 left-4 right-4 bg-[#0c1220]/95 backdrop-blur-xl border border-white/[0.08] rounded-2xl p-4 flex flex-col gap-1 shadow-[0_4px_30px_rgba(0,0,0,0.5)]">
          {navLinks.map((link) => (
            <a
              key={link}
              href={`#${link.toLowerCase().replace(/\s+/g, '-')}`}

        ...
        <div className="space-y-3">
          {certs.map((cert, i) => (
            <div key={i} className="grid grid-cols-[1fr_1fr_1fr_auto] gap-3 items-center">
              <input value={cert.icon || ''} onChange={(e) => { const c = [...certs]; c[i] = { ...c[i], icon: e.target.value }; setCerts(c); }} className="px-3 py-2 rounded-lg bg-gray-50 dark:bg-white/[0.03] border border-gray-200 dark:border-white/[0.06] text-gray-900 dark:text-white text-sm focus:outline-none" placeholder="🏆" />
              ...
              <button onClick={() => setCerts(certs.filter((_, idx) => idx !== i))} className="w-8 h-8 rounded-lg bg-red-500/10 text-red-400 flex items-center justify-center hover:bg-red-500/20 transition-all cursor-pointer">×</button>
            </div>
          ))}

...
const emptyProject = {
  title: '', description: '', longDescription: '', coverImage: '', location: '', date: '', screenshots: [], tags: [], category: 'Full Stack',
  color: 'from-blue-500/20 to-cyan-500/20', accent: '#60A5FA',
  github: '#', live: '#', period: '',
};
...
const ImageManager = ({ screenshots = [], onChange }) => {
  ...
  const updateImage = (index, field, value) => {
    const updated = screenshots.map((s, i) => i === index ? { ...s, [field]: value } : s);
    onChange(updated);
  };
  ...
  return (
    <div className="space-y-4">
      ...
      {screenshots.map((shot, index) => (
        <div key={index} className="p-4 rounded-xl bg-white/[0.02] border border-white/[0.06] space-y-3">
          ...
          {shot.url && (
            <div className="rounded-lg overflow-hidden border border-white/[0.06] max-h-40">
              <img src={shot.url} alt={`Screenshot ${index + 1}`} className="w-full h-full object-cover" />
            </div>
          )}
    ...
    <div className="space-y-2">
      ...
      {value && (
        <div className="rounded-lg overflow-hidden border border-white/[0.06] max-h-32">
          <img src={value} alt="Cover preview" className="w-full h-full object-cover" onError={(e) => e.target.style.display='none'} />
        </div>
      )}
    </div>
  );
  ...
  return (
    <div className="p-5 rounded-2xl bg-white/[0.03] border border-blue-500/20 space-y-4">
      <div className="grid sm:grid-cols-2 gap-4">
        <AdminInput label="Title" value={form.title} onChange={(v) => set('title', v)} />
        <AdminInput label="Category" value={form.category} onChange={(v) => set('category', v)} placeholder="Full Stack / Frontend / Backend" />
        <AdminInput label="Period" value={form.period} onChange={(v) => set('period', v)} placeholder="May – Jun 2026" />
        <AdminInput label="Accent Color" value={form.accent} onChange={(v) => set('accent', v)} placeholder="#60A5FA" />
        <AdminInput label="GitHub URL" value={form.github} onChange={(v) => set('github', v)} />
        <AdminInput label="Live URL" value={form.live} onChange={(v) => set('live', v)} />
      </div>

      <AdminTextarea label="Description (Short)" value={form.description} onChange={(v) => set('description', v)} rows={3} />
      <AdminTextarea label="Long Description (Detail page mein dikhega)" value={form.longDescription} onChange={(v) => set('longDescription', v)} rows={5} placeholder="Poora project ka description yahan daalo..." />

      <div className="grid sm:grid-cols-2 gap-4">
        <ImageUploadInput label="Cover Image" value={form.coverImage} onChange={(v) => set('coverImage', v)} />
        <AdminInput label="Location" value={form.location} onChange={(v) => set('location', v)} placeholder="Gorakhpur, UP" />
        <AdminInput label="Date" value={form.date} onChange={(v) => set('date', v)} placeholder="15 / 10 / 2025" />
        <AdminInput label="Card Gradient" value={form.color} onChange={(v) => set('color', v)} placeholder="from-blue-500/20 to-cyan-500/20" />
      </div>
      ...
      <div className="flex gap-3 justify-end">
        <button onClick={onCancel} className="px-4 py-2 rounded-xl text-gray-400 hover:text-white bg-white/[0.03] border border-white/[0.06] hover:bg-white/[0.06] text-sm transition-all cursor-pointer">Cancel</button>
        <SaveButton onClick={() => onSave(form)} loading={loading} label="Save Project" />
      </div>
    </div>
  );
};
...
const ProjectsEditor = () => {
  ...
  const handleAdd = async (form) => {
    setLoading(true);
    try {
      const res = await authFetch('/api/portfolio/projects', { method: 'POST', body: JSON.stringify(form) });
      const data = await res.json();
      if (data.success) { showToast('Project added!'); setAdding(false); fetchPortfolio(); }
    } catch { showToast('Error adding project.'); }
  ...
  const handleUpdate = async (id, form) => {
    setLoading(true);
    try {
      const res = await authFetch(`/api/portfolio/projects/${id}`, { method: 'PUT', body: JSON.stringify(form) });
      const data = await res.json();
      if (data.success) { showToast('Project updated!'); setEditingId(null); fetchPortfolio(); }
  ...
  const handleDelete = async (id) => {
    ...
    setLoading(true);
    try {
      const res = await authFetch(`/api/portfolio/projects/${id}`, { method: 'DELETE' });
      const data = await res.json();
      if (data.success) { showToast('Project deleted!'); fetchPortfolio(); }
  ...
  return (
    <div>
      ...
      <div className="space-y-5">
        {/* Add New */}
        {adding ? (
          <ProjectForm project={emptyProject} onSave={handleAdd} onCancel={() => setAdding(false)} loading={loading} />
        ) : (
          <button
        ...
        {projects.length === 0 && !adding && (
          <p className="text-center text-gray-600 text-sm py-6">Koi projects nahi hain. Upar "Add New Project" pe click karo.</p>
        )}
      </div>

...
const Home = () => {
  ...
  const roles = hero.roles?.length ? hero.roles : ['Full Stack Developer', 'MERN Stack Developer', 'NPM Open Source Contributor', 'Problem Solver'];
  const techStack = hero.techStack?.length ? hero.techStack : [
    { name: 'React', color: '#61DAFB' }, { name: 'Node.js', color: '#68A063' },
    { name: 'MongoDB', color: '#47A248' }, { name: 'Express', color: '#FFFFFF' },
    { name: 'Redis', color: '#FF4438' }, { name: 'AWS EC2', color: '#FF9900' },
  ];
  const stats = hero.stats?.length ? hero.stats : [
    { number: '5+', label: 'Projects Deployed' }, { number: '1+', label: 'Years Experience' },
    { number: '1400+', label: 'NPM Weekly Users' }, { number: '2', label: 'NPM Packages' },
  ];
  ...
  const finalSocialLinks = hero.socialLinks?.length
    ...
    : [
        { name: 'GitHub', href: hero.githubUrl || 'https://github.com/Shivam-75', icon: githubSvg },
        { name: 'LinkedIn', href: hero.linkedinUrl || '#', icon: linkedinSvg },
        { name: 'Email', href: `mailto:${hero.email || 'shivam6386000621@gmail.com'}`, icon: emailSvg },
        { name: 'NPM', href: 'https://www.npmjs.com/~shivam-75', icon: npmSvg },
      ];
  useEffect(() => {
        ...
        <div className="flex flex-col lg:flex-row items-center gap-12 lg:gap-16">
          ...
          <div className="flex-1 text-center lg:text-left order-2 lg:order-1">
            ...
            <p className="text-gray-400 text-base sm:text-lg leading-relaxed max-w-xl mx-auto lg:mx-0 mb-8 animate-fade-in-up animation-delay-300">
              {hero.description || 'Full-Stack Developer with 1+ year of experience building production-grade MERN Stack apps. NPM Verified Open Source Contributor with 1,400+ weekly active users.'}
            </p>

    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6">
      ...
      <div className="relative z-10 w-full max-w-6xl max-h-[90vh] bg-white dark:bg-[#0c1220] rounded-3xl shadow-2xl overflow-hidden flex flex-col border border-gray-200 dark:border-white/[0.08]">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200 dark:border-white/[0.08]">
          <h2 className="text-xl sm:text-2xl font-bold text-blue-600 dark:text-blue-400">{project.title}</h2>
          <button
            onClick={onClose}
              ...
              <div className="rounded-2xl overflow-hidden shadow-lg border border-gray-200 dark:border-white/[0.05]">
                {project.coverImage ? (
                  ...
                  <div className="w-full aspect-video bg-gradient-to-br from-blue-100 to-cyan-100 dark:from-blue-900/40 dark:to-cyan-900/40 flex items-center justify-center">
                    <span className="text-blue-500 dark:text-blue-400 font-semibold text-lg">No Cover Image</span>
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="lg:w-1/2 space-y-6">
              ...
              <div className="space-y-3 pt-4 border-t border-gray-200 dark:border-white/[0.08]">
                {project.location && (
                  <div className="flex items-center gap-3 text-gray-600 dark:text-gray-400">
                    ...
                    <span className="font-semibold text-gray-900 dark:text-white">Location:</span> {project.location}
                  </div>
                )}
                {project.date && (
                  <div className="flex items-center gap-3 text-gray-600 dark:text-gray-400">
                    <span className="text-blue-500">📅</span>
                    <span className="font-semibold text-gray-900 dark:text-white">Date:</span> {project.date}
                  </div>
                )}
              </div>
              ...
              <div className="flex flex-wrap gap-4 pt-4">
                {project.live && project.live !== '#' && (
                  <a href={project.live} target="_blank" rel="noopener noreferrer" className="px-6 py-2.5 rounded-xl bg-blue-600 text-gray-900 dark:text-white font-medium hover:bg-blue-700 hover:shadow-lg transition-all">
                    Live Demo
                  </a>
                )}
                {project.github && project.github !== '#' && (
                  <a href={project.github} target="_blank" rel="noopener noreferrer" className="px-6 py-2.5 rounded-xl bg-gray-200 dark:bg-white/[0.05] text-gray-800 dark:text-white font-medium border border-gray-300 dark:border-white/[0.1] hover:bg-gray-300 dark:hover:bg-white/[0.1] transition-all flex items-center gap-2">
